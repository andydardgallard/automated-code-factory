...
- **AGENTS.md — единый источник правды**: фабрика генерирует его с ровно 8 секциями `##` и
  ...
  первого уровня видит контентный хэш). Контентный хэш читает git-индекс, поэтому unstaged-правки
  ему не видны — об этом фабрика предупреждает, а не молчит: `--content`/`--all` печатают
  stderr-заметку, `check_factory_model.py` — warning (хэши и exit-коды не меняются; стейдж правки
  делает её видимой). Сабагенты читают AGENTS.md вместо повторного вывода структуры.
