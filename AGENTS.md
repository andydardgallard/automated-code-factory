...
- **AGENTS.md — единый источник правды**: фабрика генерирует его с ровно 8 секциями `##` и
  ...
  CI-конфиги, README, список каталогов) + контентный (SHA-256 содержимого отслеживаемых файлов
  рабочего дерева, CRLF→LF) —
  ...
  первого уровня видит контентный хэш). Контентный хэш считается по рабочему дереву `git ls-files`,
  поэтому unstaged-правки ему видны; вне хэша остаются untracked-файлы — об этом фабрика
  предупреждает, а не молчит: `--content`/`--all` печатают stderr-заметку,
  `check_factory_model.py` — warning (хэши и exit-коды не меняются; `git add` вводит
  untracked-файл в хэш). Сабагенты читают AGENTS.md вместо повторного вывода структуры.
