...
- `memory/` — переносимая долгосрочная память проекта (коммитится): `change-log.md`
  (append-only журнал прогонов) и `summary.md` (сжатая сводка)
...
- **Долгосрочная память `memory/`** (коммитится, не игнорируется): единственный писатель —
  главный агент (одна запись в `change-log.md` на прогон, компакция в `summary.md` по порогу
  50 записей); читается в начале задачи и ролями по необходимости. Проверка модели —
